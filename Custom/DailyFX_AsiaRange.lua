...
function Init()
    strategy:name("DailyFX_AsiaRange");
    strategy:description("An RSI Strategy specially designed for use during Asia trading hours.");
    ...
    strategy.parameters:addString("StartTime", "StartTime", "Start trading time", "14:00");
    strategy.parameters:addString("StopTime", "StopTime", "Stop trading time", "06:00");
...
function Prepare(nameOnly)
    ...
    local name = profile:id() .. "(" .. instance.bid:instrument() .. ", " .. tostring(LotSize) .. ", " .. tostring(StopLoss) .. ", " .. tostring(TakeProfit) .. ", " .. tostring(RSIPeriod) .. ", " .. tostring(RSIOverbought) .. ", " .. tostring(RSIOversold) .. ", " .. StartTime .. ", " .. StopTime .. ", " .. tostring(MagicNumber) .. ")";
    instance:name(name);
    ...
    CustomTXT = "PSS_DailyFX_AsiaRange_"..MagicNumber;
...
function ExtUpdate(id, source, period)
    ...
    if IsTradingTime(source, period) then
...
function IsTradingTime(source, period)
    local currentTime = source:date(NOW);
    local startTimeToday = ConvertStrToTime(StartTime, source, period);
    local endTimeToday = ConvertStrToTime(StopTime, source, period);
...
function ConvertStrToTime(timeString, source, period)
    ...
    for HH, MM in string.gmatch(timeString, "(%w+):(%w+)") do
        h = tonumber(HH);
        m = tonumber(MM);
    end
    local time = core.datetime(nowTable.year, nowTable.month, nowTable.day, h, m, 0);
    return time;
end
